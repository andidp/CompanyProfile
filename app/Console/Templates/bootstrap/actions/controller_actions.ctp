 ...
 * @return void
 */
	public function <?php echo $admin; ?>status($id = null, $status = 1) {
		if (!$this->request->is('post')) {
			throw new MethodNotAllowedException();
		}
		$this-><?php echo $currentModelName; ?>->id = $id;
		...
		if ($this-><?php echo $currentModelName; ?>->saveField('status', $status)) {
<?php if ($wannaUseSession): ?>
                        if ($status == 1) {
                            $this->Session->setFlash(__('<?php echo ucfirst(strtolower($singularHumanName)); ?> activated'), 'success');
			} else {
                            $this->Session->setFlash(__('<?php echo ucfirst(strtolower($singularHumanName)); ?> not activated'), 'error');
                        }
                        $this->redirect(array('action' => 'index'));
<?php else: ?>
			$this->flash(__('<?php echo ucfirst(strtolower($singularHumanName)); ?> deleted'), array('action' => 'index'));
