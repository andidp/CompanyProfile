 ...
 */
	public function admin_add() {
		if ($this->request->is('post')) {
			...
			$this->request->data['Post']['post_type'] = 'news';
			$this->Post->create();
 ...
 */
	public function admin_status($id = null, $status = 1) {
		...
		if ($this->Post->saveField('status', $status)) {
            if ($status == 1) {
                $this->Session->setFlash(__('News was set to publish'), 'success');
			} else {
                $this->Session->setFlash(__('News was set to draft'), 'error');
            }
            $this->redirect(array('action' => 'index'));
		}
		$this->Session->setFlash(__('News status was not change'), 'error');
		$this->redirect(array('action' => 'index'));
	}
}
